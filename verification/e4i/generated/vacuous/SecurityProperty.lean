...
def dslGuarantee_certificate_vacuous_payload (ctx : ScriptContext) : Bool :=
  (match (certificateOf ctx) with | .TxCertPoolRetire v0_0 v0_1 => (v0_1 <= 100) | _ => false)

def dslProperty_certificate_vacuous_payload (ctx : ScriptContext) : Prop :=
  dslGuarantee_certificate_vacuous_payload ctx = true
