...
def dslGuarantee_certificate_registration_deposit (ctx : ScriptContext) : Bool :=
  (match (certificateOf ctx) with | .TxCertRegDRep v0_0 v0_1 => (v0_1 == 1) | _ => false)

def dslProperty_certificate_registration_deposit (ctx : ScriptContext) : Prop :=
  dslGuarantee_certificate_registration_deposit ctx = true
