...
instance : IsData Action where
  toData x := mkDataConstr 0 [IsData.toData x.nextSequence]
  fromData
  | Data.Constr 0 [r_nextSequence] =>
      match IsData.fromData r_nextSequence with
      | some nextSequence => some { nextSequence := nextSequence }
      | _ => none
  | _ => none
...
instance : IsData State where
  toData x := mkDataConstr 0 [IsData.toData x.sequence]
  fromData
  | Data.Constr 0 [r_sequence] =>
      match IsData.fromData r_sequence with
      | some sequence => some { sequence := sequence }
      | _ => none
  | _ => none
