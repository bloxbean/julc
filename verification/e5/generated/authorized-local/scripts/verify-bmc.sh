...
trap 'rm -f "${raw}"' EXIT
set +e
lake env lean StateMachineBmc.lean >"${raw}" 2>&1
lean_status=$?
set -e
cat "${raw}"
...
if grep -Fq "No counterexample up to Depth 1" "${raw}"; then
  echo "JULC-STATE-MACHINE-V1 BOUNDED-NO-COUNTEREXAMPLE depth=1"
  exit 0
fi
if grep -Fq "Counterexample detected at Depth" "${raw}"; then
  echo "JULC-STATE-MACHINE-V1 BOUNDED-COUNTEREXAMPLE max-depth=1"
  exit 3
fi
echo "JULC-STATE-MACHINE-V1 COULD-NOT-EVALUATE strategy=BMC" >&2
exit 2
