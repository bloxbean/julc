...
trap 'rm -f "${raw}"' EXIT
set +e
lake env lean StateMachineKInduction.lean >"${raw}" 2>&1
lean_status=$?
set -e
cat "${raw}"
...
if grep -Fq "✅ Valid" "${raw}"; then
  echo "JULC-STATE-MACHINE-V1 SMT-K-INDUCTIVE max-depth=1"
  exit 0
fi
if grep -Fq "Counterexample detected at Depth" "${raw}"; then
  echo "JULC-STATE-MACHINE-V1 BASE-COUNTEREXAMPLE max-depth=1"
  exit 3
fi
if grep -Fq "Counterexample to Induction" "${raw}"; then
  echo "JULC-STATE-MACHINE-V1 INDUCTION-STEP-COUNTEREXAMPLE max-depth=1"
  exit 4
fi
echo "JULC-STATE-MACHINE-V1 COULD-NOT-EVALUATE strategy=K_INDUCTION" >&2
exit 2
