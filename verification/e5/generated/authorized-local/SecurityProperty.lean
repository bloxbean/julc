...
def redeemerStrictlyDecodes (ctx : ScriptContext) : Bool :=
  match (IsData.fromData ctx.scriptContextRedeemer :
      Option JulcGenerated.Schemas.Action) with
  | some _ => true
  | none => false
...
def typedDatum (ctx : ScriptContext) : Option JulcGenerated.Schemas.State :=
  match ctx.scriptContextScriptInfo with
  | .SpendingScript _ (some datumData) => IsData.fromData datumData
  | _ => none

def typedRedeemer (ctx : ScriptContext) : Option JulcGenerated.Schemas.Action :=
  IsData.fromData ctx.scriptContextRedeemer

def dslGuarantee_state_initial (ctx : ScriptContext) : Bool :=
  (match (typedDatum ctx) with | some v90 => (v90.sequence == 0) | none => false)

def dslProperty_state_initial (ctx : ScriptContext) : Prop :=
  dslGuarantee_state_initial ctx = true

def dslGuarantee_state_assumption (ctx : ScriptContext) : Bool :=
  (match (typedRedeemer ctx) with | some v91 => (match (typedDatum ctx) with | some v90 => true | none => false) | none => false)

def dslProperty_state_assumption (ctx : ScriptContext) : Prop :=
  dslGuarantee_state_assumption ctx = true

def dslGuarantee_state_invariant (ctx : ScriptContext) : Bool :=
  (match (typedDatum ctx) with | some v90 => (v90.sequence >= 0) | none => false)

def dslProperty_state_invariant (ctx : ScriptContext) : Prop :=
  dslGuarantee_state_invariant ctx = true

def dslGuarantee_state_transition_invariant (ctx : ScriptContext) : Bool :=
  (match (typedDatum ctx) with | some v90 => (match (typedRedeemer ctx) with | some v91 => (match (typedDatum ctx) with | some v92 => (v92.sequence == ((v90.sequence) + (1))) | none => false) | none => false) | none => false)

def dslProperty_state_transition_invariant (ctx : ScriptContext) : Prop :=
  dslGuarantee_state_transition_invariant ctx = true
...
def stateMachineInitial
    (state : JulcGenerated.Schemas.State) : Bool :=
  (state.sequence == 0)

def stateMachineAssumption
    (state : JulcGenerated.Schemas.State)
    (event : JulcGenerated.Schemas.Action)
    (ctx : ScriptContext) : Bool :=
  true

def stateMachineInvariant
    (state : JulcGenerated.Schemas.State) : Bool :=
  (state.sequence >= 0)

def stateMachineTransitionInvariant
    (state : JulcGenerated.Schemas.State)
    (event : JulcGenerated.Schemas.Action)
    (successor : JulcGenerated.Schemas.State)
    (ctx : ScriptContext) : Bool :=
