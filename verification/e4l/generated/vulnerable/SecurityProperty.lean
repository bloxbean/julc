...
def typedDatum (ctx : ScriptContext) : Option JulcGenerated.Schemas.VulnerableReviewedAdaptersDatum :=
  match ctx.scriptContextScriptInfo with
...
def dslGuarantee_reviewed_time_vulnerable (ctx : ScriptContext) : Bool :=
  (match (typedDatum ctx) with | some v0 => julcValidityContains (v0.deadline) ((ctx).scriptContextTxInfo).txInfoValidRange | none => false)

def dslProperty_reviewed_time_vulnerable (ctx : ScriptContext) : Prop :=
  dslGuarantee_reviewed_time_vulnerable ctx = true
