...
structure AuthorizedReviewedAdaptersDatum where
  deadline : Integer
deriving Repr

instance : IsData AuthorizedReviewedAdaptersDatum where
  toData x := mkDataConstr 0 [IsData.toData x.deadline]
  fromData
  | Data.Constr 0 [r_deadline] =>
      match IsData.fromData r_deadline with
      | some deadline => some { deadline := deadline }
      | _ => none
  | _ => none

inductive AuthorizedReviewedAdaptersRedeemer where
  | Redeemer
deriving Repr

instance : IsData AuthorizedReviewedAdaptersRedeemer where
  toData
