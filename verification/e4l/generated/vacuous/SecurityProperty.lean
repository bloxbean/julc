...
def typedDatum (ctx : ScriptContext) : Option JulcGenerated.Schemas.VacuousReviewedAdaptersDatum :=
  match ctx.scriptContextScriptInfo with
...
def dslGuarantee_reviewed_non_vacuity_control (ctx : ScriptContext) : Bool :=
  (match (typedDatum ctx) with | some v0 => julcValidityContains (v0.deadline) ((ctx).scriptContextTxInfo).txInfoValidRange | none => false)

def dslProperty_reviewed_non_vacuity_control (ctx : ScriptContext) : Prop :=
  dslGuarantee_reviewed_non_vacuity_control ctx = true
