...
def typedDatum (ctx : ScriptContext) : Option JulcGenerated.Schemas.AuthorizedReviewedAdaptersDatum :=
  match ctx.scriptContextScriptInfo with
...
def dslGuarantee_reviewed_time_and_authority (ctx : ScriptContext) : Bool :=
  (match (typedDatum ctx) with | some v0 => (julcAllSigned ([("\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41\x41" : ByteString)]) ctx.scriptContextTxInfo.txInfoSignatories && julcValidityContains (v0.deadline) ((ctx).scriptContextTxInfo).txInfoValidRange) | none => false)

def dslProperty_reviewed_time_and_authority (ctx : ScriptContext) : Prop :=
  dslGuarantee_reviewed_time_and_authority ctx = true
