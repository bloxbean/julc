...
def dslGuarantee_ledger_context_vacuous (ctx : ScriptContext) : Bool :=
  !(List.isEmpty (((⟨((ctx).scriptContextTxInfo).txInfoReferenceInputs⟩ : JulcList TxInInfo)).items))

def dslProperty_ledger_context_vacuous (ctx : ScriptContext) : Prop :=
  dslGuarantee_ledger_context_vacuous ctx = true
