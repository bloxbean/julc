...
def dslGuarantee_ledger_context_vulnerable (ctx : ScriptContext) : Bool :=
  (List.any (((⟨((ctx).scriptContextTxInfo).txInfoReferenceInputs⟩ : JulcList TxInInfo)).items) (fun v0 => (match (((v0).txInInfoResolved).txOutDatum) with | .OutputDatum .. => true | _ => false)) && (match (findOwnInput (ctx)) with | some _ => true | none => false))

def dslProperty_ledger_context_vulnerable (ctx : ScriptContext) : Prop :=
  dslGuarantee_ledger_context_vulnerable ctx = true
