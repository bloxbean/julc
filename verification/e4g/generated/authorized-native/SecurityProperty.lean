...
def dslGuarantee_ledger_context_current_purpose (ctx : ScriptContext) : Bool :=
  (match ((ctx).scriptContextScriptInfo.toScriptPurpose) with | .Spending .. => true | _ => false)

def dslProperty_ledger_context_current_purpose (ctx : ScriptContext) : Prop :=
  dslGuarantee_ledger_context_current_purpose ctx = true

def dslGuarantee_ledger_context_datum_witness (ctx : ScriptContext) : Bool :=
  List.any (((⟨((ctx).scriptContextTxInfo).txInfoData⟩ : JulcMap CardanoLedgerApi.V2.DatumHash Data)).entries) (fun entry => let v0 := entry.1; let v1 := entry.2; true)

def dslProperty_ledger_context_datum_witness (ctx : ScriptContext) : Prop :=
  dslGuarantee_ledger_context_datum_witness ctx = true

def dslGuarantee_ledger_context_fee (ctx : ScriptContext) : Bool :=
  (((ctx).scriptContextTxInfo).txInfoFee >= 0)

def dslProperty_ledger_context_fee (ctx : ScriptContext) : Prop :=
  dslGuarantee_ledger_context_fee ctx = true

def dslGuarantee_ledger_context_redeemer_witness (ctx : ScriptContext) : Bool :=
  List.any (((⟨((ctx).scriptContextTxInfo).txInfoRedeemers⟩ : JulcMap ScriptPurpose Data)).entries) (fun entry => let v0 := entry.1; let v1 := entry.2; true)

def dslProperty_ledger_context_redeemer_witness (ctx : ScriptContext) : Prop :=
  dslGuarantee_ledger_context_redeemer_witness ctx = true

def dslGuarantee_ledger_context_reference_shape (ctx : ScriptContext) : Bool :=
  (match (julcListAt (((⟨((ctx).scriptContextTxInfo).txInfoReferenceInputs⟩ : JulcList TxInInfo)).items) (0)) with | some v0 => (((match (((v0).txInInfoResolved).txOutDatum) with | .OutputDatum .. => true | _ => false) && (match ((((v0).txInInfoResolved).txOutAddress).addressCredential) with | .ScriptCredential .. => true | _ => false)) && (match (((v0).txInInfoResolved).txOutReferenceScript) with | some _ => false | none => true)) | none => false)

def dslProperty_ledger_context_reference_shape (ctx : ScriptContext) : Prop :=
  dslGuarantee_ledger_context_reference_shape ctx = true
