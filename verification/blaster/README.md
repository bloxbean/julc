...
Run the reviewed suite through the managed C.4 result protocol:

```bash
julc verify run verification/blaster --backend local
```

This writes `verification/blaster/verification-result.json` and phase logs
under `verification-results/`. The repository suite remains local-only because
its legacy acquisition step rebuilds fixtures outside this workspace; Docker is
available for workspaces generated by `julc verify init`.
