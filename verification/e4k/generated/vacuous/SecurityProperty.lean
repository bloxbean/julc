...
def dslGuarantee_governance_non_vacuity_control (ctx : ScriptContext) : Bool :=
  (match (julcListAt (((⟨((ctx).scriptContextTxInfo).txInfoProposalProcedures⟩ : JulcList ProposalProcedure)).items) (0)) with | some v0 => ((v0).ppDeposit >= 10) | none => false)

def dslProperty_governance_non_vacuity_control (ctx : ScriptContext) : Prop :=
  dslGuarantee_governance_non_vacuity_control ctx = true
