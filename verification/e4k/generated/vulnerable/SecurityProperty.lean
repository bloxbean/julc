...
def dslGuarantee_governance_hard_fork_v11 (ctx : ScriptContext) : Bool :=
  (match (julcListAt (((⟨((ctx).scriptContextTxInfo).txInfoProposalProcedures⟩ : JulcList ProposalProcedure)).items) (0)) with | some v0 => (((v0).ppDeposit >= 10) && (match ((IsData.fromData (v0).ppGovernanceAction : Option GovernanceAction)) with | some v1 => (match (v1) with | .HardForkInitiation v2_0 v2_1 => ((v2_1).pvMajor == 11) | _ => false) | none => false)) | none => false)

def dslProperty_governance_hard_fork_v11 (ctx : ScriptContext) : Prop :=
  dslGuarantee_governance_hard_fork_v11 ctx = true
