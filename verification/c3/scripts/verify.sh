...
lake_update_with_retry() {
  local attempt
  for attempt in 1 2 3; do
    if lake update; then
      return 0
    fi
    echo "Lake dependency acquisition failed (attempt ${attempt}/3); retrying" >&2
  done
  echo "COULD-NOT-EVALUATE: unable to acquire pinned Lake dependencies" >&2
  return 2
}
