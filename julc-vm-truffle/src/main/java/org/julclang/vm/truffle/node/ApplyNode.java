...
public final class ApplyNode extends UplcNode {
    ...
    static String describeValue(Object v) {
        if (v instanceof org.julclang.core.Constant c) return "Constant(" + c.type() + ")";
        if (v instanceof UplcClosure) return "Closure";
        if (v instanceof org.julclang.vm.truffle.runtime.UplcDelay) return "Delay";
        if (v instanceof UplcBuiltinDescriptor b) return "Builtin(" + b.getFun() + ")";
        if (v instanceof org.julclang.vm.truffle.runtime.UplcConstrValue c) return "Constr(" + c.getTag() + ")";
