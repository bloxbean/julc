...
/**
 * Integration tests: JRL source -> UPLC -> VM evaluation -> correct result.
...
class JrlIntegrationTest {
    ...
    private Program compileJrl(String jrlSource) {
        return JrlTestSupport.compileJrl(jrlSource);
    }
    ...
    class ErrorHandlingTests {
        ...
        void parseError_returnsDiagnostics() {
            var compiler = new JrlCompiler();
            var result = compiler.compile("not valid JRL", "bad.jrl");
            assertTrue(result.hasErrors());
            assertFalse(result.jrlDiagnostics().isEmpty());
        }
        ...
        void typeError_returnsDiagnostics() {
            ...
            var result = compiler.compile("""
                    ...
                    """, "test.jrl");
            assertTrue(result.hasErrors());
            assertTrue(result.jrlDiagnostics().stream()
                    .anyMatch(d -> d.code().equals("JRL002")));
        }
