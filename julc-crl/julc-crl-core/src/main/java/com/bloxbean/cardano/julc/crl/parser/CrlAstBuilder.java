...
import com.bloxbean.cardano.julc.crl.grammar.CRLBaseVisitor;
...
/**
 * Converts an ANTLR CRL parse tree into a clean, immutable AST.
 * <p>
 ...
 */
public class CrlAstBuilder extends CRLBaseVisitor<Object> {

    private final String fileName;

    public CrlAstBuilder(String fileName) {
    ...
    @Override
    public ContractNode visitCrlFile(CRLParser.CrlFileContext ctx) {
        // Header
    ...
    @Override
    public PurposeType visitPurposeType(CRLParser.PurposeTypeContext ctx) {
    ...
    @Override
    public ParamNode visitParamDecl(CRLParser.ParamDeclContext ctx) {
    ...
    public TypeRef visitTypeRef(CRLParser.TypeRefContext ctx) {
        if (ctx instanceof CRLParser.ListTypeContext lt) {
            return new TypeRef.ListType(visitTypeRef(lt.typeRef()), rangeOf(ctx));
        } else if (ctx instanceof CRLParser.OptionalTypeContext ot) {
            return new TypeRef.OptionalType(visitTypeRef(ot.typeRef()), rangeOf(ctx));
        } else if (ctx instanceof CRLParser.SimpleTypeContext st) {
    ...
    @Override
    public DatumDeclNode visitDatumDecl(CRLParser.DatumDeclContext ctx) {
    ...
    @Override
    public RecordDeclNode visitRecordDecl(CRLParser.RecordDeclContext ctx) {
        List<FieldNode> fields = ctx.fieldDecl().stream()
                .map(this::visitFieldDecl).toList();
    ...
    @Override
    public FieldNode visitFieldDecl(CRLParser.FieldDeclContext ctx) {
    ...
    @Override
    public RedeemerDeclNode visitRedeemerDecl(CRLParser.RedeemerDeclContext ctx) {
        String name = ctx.IDENT().getText();
        var body = ctx.redeemerBody();
        if (body instanceof CRLParser.VariantRedeemerContext vr) {
        ...
        } else if (body instanceof CRLParser.RecordRedeemerContext rr) {
    ...
    @Override
    public VariantNode visitVariantDecl(CRLParser.VariantDeclContext ctx) {
    ...
    @Override
    public PurposeSectionNode visitPurposeSection(CRLParser.PurposeSectionContext ctx) {
    ...
    @Override
    public RuleNode visitRuleDecl(CRLParser.RuleDeclContext ctx) {
    ...
    @Override
    public DefaultAction visitDefaultDecl(CRLParser.DefaultDeclContext ctx) {
        return ctx.action().ALLOW() != null ? DefaultAction.ALLOW : DefaultAction.DENY;
    }

    // ── Fact patterns ───────────────────────────────────────────

    private FactPattern visitFactPatternDispatch(CRLParser.FactPatternContext ctx) {
        if (ctx instanceof CRLParser.RedeemerPatternContext rp) {
            return new FactPattern.RedeemerPattern(
                    visitVariantMatch(rp.variantMatch()), rangeOf(ctx));
        } else if (ctx instanceof CRLParser.TransactionPatternContext tp) {
        ...
        } else if (ctx instanceof CRLParser.DatumPatternContext dp) {
            return new FactPattern.DatumPattern(
                    visitVariantMatch(dp.variantMatch()), rangeOf(ctx));
        } else if (ctx instanceof CRLParser.ConditionPatternContext cp) {
            return new FactPattern.ConditionPattern(
                    visitExprDispatch(cp.expr()), rangeOf(ctx));
        } else if (ctx instanceof CRLParser.OutputPatternContext op) {
            return buildOutputPattern(op);
        }
        throw new IllegalStateException("Unknown fact pattern: " + ctx.getText());
    }

    private FactPattern.OutputPattern buildOutputPattern(CRLParser.OutputPatternContext ctx) {
        Expression to = null;
        ValueConstraint value = null;
        DatumConstraint datum = null;

        for (var field : ctx.outputField()) {
            if (field instanceof CRLParser.OutputToContext ot) {
                to = visitExprDispatch(ot.expr());
            } else if (field instanceof CRLParser.OutputValueContext ov) {
                value = visitValueExpr(ov.valueExpr());
            } else if (field instanceof CRLParser.OutputDatumContext od) {
    ...
    private ValueConstraint visitValueExpr(CRLParser.ValueExprContext ctx) {
        if (ctx instanceof CRLParser.MinAdaValueContext mv) {
            return new ValueConstraint.MinADA(visitExprDispatch(mv.expr()));
        } else if (ctx instanceof CRLParser.ContainsValueContext cv) {
    ...
    private DatumConstraint buildDatumConstraint(CRLParser.DatumExprContext ctx) {
    ...
    @Override
    public VariantMatch visitVariantMatch(CRLParser.VariantMatchContext ctx) {
        String typeName = ctx.IDENT().getText();
    ...
    @Override
    public FieldMatch visitFieldMatch(CRLParser.FieldMatchContext ctx) {
    ...
    private MatchValue visitMatchValue(CRLParser.MatchValueContext ctx) {
        if (ctx instanceof CRLParser.BindingValueContext bv) {
            String varName = bv.VAR_BINDING().getText().substring(1); // strip '$'
            return new MatchValue.Binding(varName, rangeOf(ctx));
        } else if (ctx instanceof CRLParser.NestedMatchValueContext nm) {
        ...
        } else if (ctx instanceof CRLParser.LiteralValueContext lv) {
    ...
    private Expression visitExprDispatch(CRLParser.ExprContext ctx) {
        if (ctx instanceof CRLParser.OrExprContext oe) {
        ...
        } else if (ctx instanceof CRLParser.AndExprContext ae) {
            return new Expression.BinaryExpr(
                    visitExprDispatch(ae.expr(0)), Expression.BinaryOp.AND,
                    visitExprDispatch(ae.expr(1)), rangeOf(ctx));
        } else if (ctx instanceof CRLParser.EqualityExprContext ee) {
        ...
        } else if (ctx instanceof CRLParser.ComparisonExprContext ce) {
        ...
        } else if (ctx instanceof CRLParser.AdditiveExprContext ad) {
