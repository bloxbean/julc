...
public class JulcTransactionEvaluator implements TransactionEvaluator {
    ...
    private volatile org.julclang.vm.JulcVmProvider provider;
        ...
        try {
            ...
            for (Redeemer redeemer : redeemers) {
                try {
                    ...
                    var evalOptions = new org.julclang.vm.EvalOptions(
    ...
    private org.julclang.vm.JulcVmProvider getProvider() {
        if (provider == null) {
            synchronized (this) {
                if (provider == null) {
                    provider = java.util.ServiceLoader.load(org.julclang.vm.JulcVmProvider.class)
                            .stream()
                            .map(java.util.ServiceLoader.Provider::get)
                            .max(java.util.Comparator.comparingInt(org.julclang.vm.JulcVmProvider::priority))
    ...
    private Program decodeScript(
        ...
        return JulcScriptAdapter.toProgram(cborHex,
                new org.julclang.vm.LedgerEvaluationTarget(
                        language,
                        new org.julclang.vm.ProtocolVersion(
                                protocolMajor, protocolMinor)));
    }
