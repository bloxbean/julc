...
import org.julclang.playground.api.InputValidator;
...
class PlaygroundServiceTest {
    ...
    static final PlaygroundService SERVICE = new PlaygroundService(COMPILER,
    ...
    void check_extractsMetadata() {
        var result = PlaygroundService.check(new CheckRequest(SIMPLE_SPENDING));
    ...
    void check_rejectsBlankSourceWithDiagnostic() {
        var result = PlaygroundService.check(new CheckRequest(" "));
    ...
    void compile_oversizedLibraryIsRejectedBeforeCompiling() {
        ...
        assertNotNull(PlaygroundService.validateCompile(new CompileRequest(ALWAYS_TRUE, null, library, false)));
        assertNull(PlaygroundService.validateCompile(new CompileRequest(ALWAYS_TRUE)));
    }
    ...
    void evalExpression_usesGivenEvaluator() {
        var evaluator = new PlaygroundEvaluator(COMPILER, JulcVm.withProvider(new JavaVmProvider()),
                LibrarySourceResolver.scanClasspathSources(JulcCompiler.class.getClassLoader()));

        var ok = PlaygroundService.evalExpression(evaluator, new EvalExpressionRequest("MathLib.abs(-5)"));
        var blank = PlaygroundService.evalExpression(evaluator, new EvalExpressionRequest(""));
    ...
    void scenarios_knownAndUnknownPurposes() {
        var spending = PlaygroundService.scenarios("spending");
        var unknown = PlaygroundService.scenarios("unknown");
