---
title: "For-Each Loop Patterns"
description: "For-Each Loop Patterns - JuLC documentation"
---
...
| General body-local reassignment in single-accumulator loops with `break` | Not supported | Prefer an initializer; accumulator assignments remain supported |
| Method-level `if` wraps a loop updating a variable read after the branch | Known miscompile | See [#161](https://github.com/bloxbean/julc/issues/161); not fixed by #157 |
| Loop reassigns a switch case-pattern variable, then reads its fields | Known miscompile | See [#162](https://github.com/bloxbean/julc/issues/162); copy to a fresh arm-local accumulator instead |
