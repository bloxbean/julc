...
| `if` outside a loop body (method level or switch arm) wraps a loop updating a variable read after the branch | Supported (#161 fix) | Updated accumulators reach following statements; switch-arm accumulators must belong to that arm |
