...
- **Known correctness limitation:** an `if` outside a loop body, at method level or inside a switch arm, may silently lose a nested loop's updates to an accumulator read after the branch ([#161](https://github.com/bloxbean/julc/issues/161)). Do not generate this pattern; #157 does not fix it.
