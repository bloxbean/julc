...
- **Known correctness limitation:** a method-level `if` wrapping a loop may silently lose updates to an enclosing accumulator read after the branch ([#161](https://github.com/bloxbean/julc/issues/161)). Do not generate this pattern; #157 does not fix it.
- **Known correctness limitation:** do not reassign a switch case-pattern variable and then read its fields ([#162](https://github.com/bloxbean/julc/issues/162)); copy it to a fresh arm-local accumulator instead.
