...
variable and source location, and suggests a conditional initializer or moving
the declaration before the loop (resetting it each iteration if necessary).

This is a conservative restriction, including constant conditions and dead updates.
Accepted programs retain the same generated bytes; existing deployed scripts are
