...
**Cause:** `julc verify` generates a managed, hash-bound workspace and refuses
to overwrite existing files implicitly.

**Fix:** Choose the operation that matches your intent:

```bash
# Source/property changed: rebuild and regenerate generator-owned files.
julc verify . --validator MyValidator --backend local --force

# Reproduce the existing exact workspace without rebuilding it.
julc verify run verification/my-validator --backend local
```

Do not delete or edit hash-bound generated files to bypass the check. Preflight
will reject a workspace whose artifact, property, plan, or generated Lean no
...
take much longer than dependency setup. Supported syntax does not guarantee
solver termination; preserve the recorded timeout or undetermined result
instead of weakening the property to obtain success.

### Expected Lean 4.24.0

**Cause:** The local backend requires the pinned Lean/Lake toolchain.

**Fix:** Install it with elan and keep the shims on `PATH`:
...
Run from the project/workspace that contains `lean-toolchain`; elan then selects
the pinned version. Alternatively use `--backend docker`.

### Z3 is missing or has the wrong version

JuLC uses the exact pinned Z3 version. When a compatible executable is not
available, the local backend downloads the official archive into
`.julc/tools`, validates its checksum, and rechecks its version. Ensure the
first acquisition run has network access and that the workspace is writable.
...
This is expected. The native CLI does not execute project specification classes
inside the native image. `julc verify dsl` launches a bounded child JVM because
the specification is trusted project Java. Install Java 25 and put the JuLC JAR
and compiled specification classes on `--spec-classpath`.
...
ledger-valid Cardano transaction. It refutes the generated Blaster-domain
solver obligation. Unless `ledgerValidCounterexampleEstablished` is true, it
does not establish a counterexample in the narrower pinned ledger-valid domain;
unless `concreteVmCounterexampleReproduced` is true, it has not been reproduced
as a concrete VM execution. In every case it requires investigation.

### `COULD-NOT-EVALUATE: property-vacuous`

The separate bounded non-vacuity SMT check established that no successful
execution exists under the property's selected modeled premise/domain and CEK
fuel. An implication over that empty successful-path set would be misleading,
so JuLC does not classify the main theorem as established. If the separate
check cannot decide whether a successful execution exists, the reason is
...
symbolic behavior, non-vacuity uncertainty, an incomplete custom property, or a
backend limitation. Keep the result and logs; do not translate it to
`SMT-VALID` in CI.
...
See the [formal verification guide](/guides/formal-verification/) for the full
workflow and result scope.
