package org.julclang.compiler.fixtures;
...
/**
 * Fixture for testing JulcMap methods with byte[] values.
 * Map type: JulcMap<PlutusData, byte[]> — value auto-unwrap via UnBData.
 */
public class ByteMapOps {

    @SuppressWarnings("unchecked")
    static byte[] get(PlutusData m, PlutusData key) {
        JulcMap<PlutusData, byte[]> map = (JulcMap) (Object) m;
        return map.get(key);
    }
    ...
    static boolean lookupPresent(PlutusData m, PlutusData key) {
        JulcMap<PlutusData, byte[]> map = (JulcMap) (Object) m;
        Optional<byte[]> result = map.lookup(key);
