...
import org.julclang.compiler.util.MethodDependencyResolver.NamedBinding;
...
class MethodOrderingTest {
    ...
    private BigInteger evalInteger(Term term) {
        var result = vm.evaluate(org.julclang.core.Program.plutusV3(term));
    ...
    class DependencyResolverTests {
        ...
        private boolean containsVar(PirTerm term, String name) {
            return switch (term) {
                ...
                case PirTerm.PairMatch m -> containsVar(m.scrutinee(), name) || containsVar(m.body(), name);
                case PirTerm.IntegerCase c -> containsVar(c.scrutinee(), name)
                        || c.branches().stream().anyMatch(b -> containsVar(b, name));
