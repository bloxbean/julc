...
public final class PlutusScriptLoader {
    ...
    public static PlutusV3Script load(Class<?> validatorClass,
        ...
        var coreParams = new com.bloxbean.cardano.plutus.core.PlutusData[params.length];
        for (int i = 0; i < params.length; i++) {
            coreParams[i] = PlutusDataAdapter.fromClientLib(params[i]);
        }
        // Decode → apply → re-encode
        var program = PlutusScriptAdapter.toProgram(output.cborHex());
        var concrete = program.applyParams(coreParams);
        return PlutusScriptAdapter.fromProgram(concrete);
    }
