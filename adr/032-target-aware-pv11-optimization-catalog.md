...
- Deferred O14 because `JulcValue` is currently produced through partial
  ...
  fold, and `NativeValueLib.empty/singleton/lovelace` are the typed producers.)
...
| O14 | [#108](https://github.com/bloxbean/julc/issues/108), [#119](https://github.com/bloxbean/julc/issues/119) | ADR-045 implemented: typed literal producers (`NativeValueLib.empty/singleton/lovelace` over a UPLC Value constant) and folding of all-literal calls of the seven Value builtins at `PV11_SAFE` (`pv11.o14.value-literal-fold`) by the pinned `NativeValueSemantics` the VM shares, under a script-size objective; failing literal calls stay; no algebraic identities; additive for existing programs |
