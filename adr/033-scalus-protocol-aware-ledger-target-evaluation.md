...
**Governing decisions:** [ADR-029 — Protocol Version 11 Ledger Readiness and Optimization Roadmap](029-pv11-ledger-readiness-and-optimization-roadmap.md) and [ADR-030 — Protocol Version Propagation and PV11 Builtin Semantics](030-protocol-version-propagation-and-pv11-builtin-semantics.md)
...
**Scalus dependency baseline:** [`org.scalus:scalus_3:1.1.0`](https://github.com/nau/scalus/tree/v1.1.0)
...
| Language-only V1/V2 | Uses Scalus's bundled defaults; supplied arrays are ignored |
...
Reachability is not certification. Scalus's V1/V2 parameter adapters include
fallback behavior for PV11 machine and builtin costs that are absent from the
parameter representation they consume. Before JuLC claims V1/PV11 or V2/PV11,
tests must prove that a supplied 332-value model controls every available
builtin and machine step exactly as the pinned Plutus evaluator does. An
upstream reference-cost substitution must not be described as use of the
caller's supplied model.
...
8. Replace the disabled/skip-heavy Scalus conformance test with an explicit
   matrix and exact budget assertions.
...
- Upgrading Scalus beyond 1.1.0 as part of issue #74.
...
or an explicitly documented bridge limitation. The runner asserts category
counts. Adding an unclassified file fails the suite.
...
### D1. Initial certified support matrix
...
|---|---|---|---|
| V3/PV10 | Required for certification | C | Matching configured model, otherwise the pinned Scalus 1.1.0 default verified by the matrix |
| V3/PV11 | Required for certification | E | Matching configured model, otherwise the pinned Scalus 1.1.0 default verified by the matrix |
...
Replace the two V3-oriented fields with one immutable record:

```java
record ConfiguredScalusVm(
...
The provider holds one `volatile ConfiguredScalusVm` slot for V1, V2, and V3.
Construction happens entirely in local variables. The matching slot is
assigned only after target resolution, parameter normalization, Scalus model
construction, and consistency checks all succeed.

An evaluation reads the relevant slot exactly once and passes that local
snapshot through the remaining call chain. This removes torn target/model
reads. It does not declare the provider fully thread-safe or close issue #40.
...
target
  ...
  -> select certified default or configured MachineParams
...
For the initially certified V3 profiles, the active target schemas are:
...
1. If the language's configuration slot is non-null and has the same Plutus
   semantics as the request, use that exact record.
2. If the slot is non-null but targets a different protocol major, fail with
   zero consumed budget.
3. If the slot is null, use Scalus 1.1.0's target-specific default parameters
   only after the exact pinned matrix has certified that default for the
   requested profile.
4. Never fall back from an unsupported profile to the newest certified one.

The no-configuration default is deterministic because the dependency version
is pinned. It is a bundled evaluator default, not a claim that the provider has
loaded the current network's governance parameters. Callers performing live
ledger validation should configure the cost model obtained with the target
protocol parameters.

### D7. Preserve and isolate the legacy language-only API

The language-only API remains a compatibility surface:

- configured V3 uses the retained V3 target and model;
- unconfigured V3 continues through Scalus's existing no-argument V3 factory;
- unconfigured V1/V2 continue through their existing no-argument factories;
- configuring V1/V2 must no longer mutate protocol state later observed by a
  V3 evaluation;
- language-only calls do not expand the explicit certified matrix.

This preserves the behavior called out in issue #74 while removing accidental
cross-language state coupling. New ledger-validation code must use the
explicit-target API.
...
The existing Array, Value, and BLS exclusions must be re-audited against
Scalus 1.1.0. Where JuLC's `Term` and `Constant` models can represent a result,
the bridge must convert it and the test must run. If JuLC cannot represent an
upstream result at all, the case may be classified as a structural
inapplicability only with:

- a named reason code;
- an asserted fixed count;
- a focused test of the closest representable use, including use as an
  intermediate value when possible;
- no effect on builtin availability validation for programs whose final result
  is representable.
...
4. **Runtime semantics:** pinned C/D/E-sensitive integer, ByteString,
   division, BLS, Array, and Value behavior matches the reference where
   applicable.
5. **Cost provenance:** both default and supplied model paths are tested; a
   parameter perturbation proves the supplied path is active.
6. **Exact budgets:** every numeric reference vector matches CPU and memory
   exactly.
...
9. **Bridge coverage:** all exclusions are named and count-asserted; there are
   no directory-name skips.
...
substitute for budget evidence.
...
| `julc-vm-scalus` bridge | Complete or explicitly classify Array/Value/BLS/result conversions needed by the matrix |
...
| ADR-029 and ADR-030 | Record the final certified Scalus matrix and remaining exclusions |
...
- Add `ConfiguredScalusVm` and per-language volatile slots.
...
- Assert corpus, applicable, numeric-budget, semantic-overlay, and exclusion
  counts.
- Remove directory-name skips and complete representable Array, Value, and BLS
  conversions.
...
- Update `ScalusVmProvider` Javadocs with the certified matrix and compatibility
  behavior.
- Update ADR-029 task PV11-036 and its readiness checklist.
- Update ADR-030's Scalus exclusion with the exact resulting matrix.
- Update issues #65, #74, and #121 with commands, counts, and any exclusions.
- Run affected consumer tests to ensure explicit-target selection works through
  `JulcVm`, testkit, and Cardano Client Lib integration.
...
- bridge conversion of every representable constant type exercised by the
  pinned corpus.
...
classification counts. Every exact-budget assertion is against the pinned
Plutus expectation. Java and Truffle results are useful cross-checks but are
...
language-only calls retain Scalus's current defaults.
...
Rejected. It would make Scalus's canonical API unexpectedly narrower than
Java and Truffle and would prevent deterministic test/evaluator defaults. The
unconfigured default is allowed only for profiles certified with the pinned
Scalus dependency and is documented as non-live-network configuration.
...
| Risk | Mitigation |
|---|---|
| Scalus semantics selection drifts in a dependency update | Pin 1.1.0 and assert VM language/protocol/variant directly plus behavioral boundaries |
| Caller-supplied costs are replaced by Scalus fallback values | Normalize explicitly, perturb individual parameters, and withhold support where provenance cannot be proved |
...
- Scalus can participate in canonical V3/PV10 and V3/PV11 evaluation with an
...
- [ ] A configured/requested major mismatch fails deterministically with zero
      budget consumed.
- [ ] Configuration is atomically published as one immutable per-language
      record.
...
- [ ] Exact reference budgets pass for every case in each certified profile's
      reviewable matrix.
- [ ] The disabled Scalus conformance runner is replaced; all exclusions have
      fixed reason codes and asserted counts.
- [ ] Array, Value, BLS, negative-constructor, and result-conversion limitations
      are either fixed or precisely documented by the final matrix.
- [ ] ADR-029, ADR-030, provider Javadocs, and issues #65/#74/#121 state the
      final verified scope.
...
1. Which Array, Value, and BLS cases remain blocked by Scalus 1.1.0 itself,
   and which are only missing `TermConverter` coverage?
2. Do Scalus 1.1.0 target-specific V3 defaults match the pinned PV10 and PV11
   conformance budgets for every applicable vector?
3. Can Scalus's V1/V2 parameter adapters consume every supplied PV11 cost
   without fallback, or must upstream support change before JuLC can certify
   those profiles?
